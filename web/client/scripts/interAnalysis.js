...
function getTheGraphs(graphSize) {
    ...
    let tempDict = {};
    let subTermsGraph = null;
    if (graphSize) {
        subTermsGraph = myVsearch.termsAllArrs.slice(0, graphSize);
    } else {
        subTermsGraph = myVsearch.termsAllArrs;
    }
...
function benterm2Document(searchResult) {
    ...
    for (index = 0, len = searchResult.length; index < len; index += 1) {
        ...
        let personList = searchResult[index].personList();

        let keywordsList = searchResult[index].keywordsList();

        for (index2 = 0, len2 = personList.length; index2 < len2; index2 += 1) {
            tempTerm = personList[index2];
            if (tempTerm === '') {
                continue;
            }

            if (res.term2Doc[tempTerm]) {
                res.term2Doc[tempTerm].push(index);
            } else {
                res.term2Doc[tempTerm] = [index];
            }

            res.personDict[tempTerm] = 0;
        }
        for (index2 = 0, len2 = keywordsList.length; index2 < len2; index2 += 1) {
            tempTerm = keywordsList[index2];
            if (tempTerm === '') {
                continue;
            }
