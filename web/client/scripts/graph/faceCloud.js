...
myFaceCloud.fn.initialMethod = function(tagID) {
    ...
    let that = this;
    notifyier.subscribe('filterTerms', 0, function(data) {
        if (that.isShowing) {
            that.isFiltered = true;
            that.feebleTerms(data);
        }
    })
        ...
        .attr('display', function(d, j) {
            if (d.isImage) {
                ...
                that.imagesDict[j].attr('x', d.x + DEMI_BORDER_SIZE)
                                    ...
                                    .attr('width', d.y1*2/3- BORDER_SIZE)
        ...
        .attr('display', function(d, i) {
            ...
            if (d.isImage) {
                ...
                that.imagesDict[i] = that.imagesPane.append('svg:image')
                                        ...
                                        .style('background-size', (d.y1*2/3) + 'px' + ' ' + d.y1 + 'px')
                                        .attr('width', (d.x1*2/3) - BORDER_SIZE)
                                        .attr('height', d.y1 - BORDER_SIZE)
...
myFaceCloud.fn.feebleTerms = function(data) {
    ...
    this.wordPane.selectAll('.words')
                ...
                .style('opacity', function(d, j) {
                    ...
                    if (data.nodeDict[d.text]) {
                        d.tempHeat = data.nodeDict[d.text]
                    } else {
                        d.tempHeat = 0;
                    }
                ...
                .style('font-size', function(d, j) {
                    ...
                    if (tvale < d.size && terms[d.text]) {
                        ...
                        if (d.isImage) {
                            ...
                            that.borderDict[j].attr('width', w*2/3)
                                                .attr('height', w);
                            that.imagesDict[j].attr('width', w*2/3 - BORDER_SIZE)
...
myFaceCloud.fn.updateHeatData = function(heatFormat) {
    ...
    for (let i = 0; i < this.myData.sizeOfMatrix; i += 1) {
        ...
        if (oneTerm.isImage) {
            ...
            x1 = oneTerm.x + oneTerm.y1*2/3 + middleX;
